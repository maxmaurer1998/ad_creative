...
- Fade overlay: direction (top/bottom), reach, speed-to-solid, an intensity cap (so the darkest point can be dialled down to a light tint instead of ever becoming a fully solid block), colour, and an optional "textured" toggle that adds a subtle procedural paper/leather-style grain, masked to fade in and out with the gradient's own alpha — using the smoothstep + plateau easing so the fade always blends to 0% opacity at its outer edge
