...
- Three independent text layers (headline, subheader, optional "other" line) — subheader and "other" can each be toggled off entirely — each with font, size, colour. The whole stacked text block can be dragged directly on the canvas to reposition it (like the logo), or moved with two continuous position sliders — one along the block's own stacking direction, one along the cross axis — always staying inside the edge padding. A horizontal-alignment control (left/center/right) still governs how each line sits within the block. An orientation control rotates the whole block 90° either way (reading top-to-bottom or bottom-to-top) for vertical text, with the sliders and their labels adapting to the rotated axes. Clicking the already-active orientation button again snaps both position sliders back to dead-centre
- Inline legibility warning banner when the text block sits over an insufficiently-faded area
- Logo upload with drag-to-position on canvas or a vertical-position slider (both clamped to the same left/right and top/bottom margins as the text, at any size), corner + center position presets, and a colour choice: keep the logo's original colours, tint it to a custom colour you pick, or tint it to always match the current fade colour. While dragging either the logo or the text block, the margin boundaries are always shown, plus "smart" alignment guides that appear only when the dragged object's edges or centre line up with the canvas centre, the margins, or the other object's own edges/centre — the same alignment signal design tools give you
