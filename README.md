...
- Effects tab: film grain (with a fine-to-coarse grain-size slider — the low end is now genuinely fine, not just less-strong), vignette, a warm "heritage" colour-grade, and a "Film stock" selector with two full grades (each with its own strength slider), applied across the whole composite (photo, fade, text and logo):
  ...
  - **Tech optics** — a stylised late-'90s "classified hardware" look: crushed blacks with a cold steel/silver split tone, chromatic aberration that strengthens toward the frame edges, a chrome highlight bloom, fine grain, a cool anamorphic flare, a vignette, and an optional corner-bracket/grid/spec-text HUD overlay (the label text is editable)
