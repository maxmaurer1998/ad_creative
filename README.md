...
- Fade overlay: direction (top/bottom), reach, speed-to-solid, an intensity cap (so the darkest point can be dialled down to a light tint instead of ever becoming a fully solid block), colour, and an optional textured toggle (with its own strength slider) that adds a procedural paper/leather-style grain masked to fade in and out with the gradient's own alpha — using the smoothstep + plateau easing so the fade always blends to 0% opacity at its outer edge
- Effects tab: film grain, vignette, and a warm "heritage" colour-grade, each an independent 0-100% slider applied across the whole composite (photo, fade, text and logo) — combine them for a premium/vintage-print finish
