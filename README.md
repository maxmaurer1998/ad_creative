...
- Position tab: pinch/drag (or a zoom slider, 50–400%) to pan and zoom the photo within the canvas. Only active while that tab is open — the photo locks in place as soon as you switch tabs, so drags elsewhere can't accidentally move it. Zooming out below 100% shrinks the photo within the frame instead of cropping it further (which isn't possible — the 100% crop already shows as much of the image as the frame's aspect ratio allows), revealing a plain white margin around it — useful when the source photo already has white space around the subject and the frame's aspect ratio doesn't match it
